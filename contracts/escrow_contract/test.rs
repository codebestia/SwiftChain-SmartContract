...
fn test_init_and_get_status() {
    ...
    let token = init_contract(&env, &client, &admin, 25);
    ...
    assert_eq!(client.get_protocol_version(), 1);
}

#[test]
fn test_init_rejects_second_initialization() {
    let (env, contract_id) = setup_env();
    let client = EscrowContractClient::new(&env, &contract_id);

    let admin = Address::generate(&env);
    let token = init_contract(&env, &client, &admin, 0);

    let result = client.try_init(&admin, &token, &0);

    match result {
        Err(Ok(err)) => assert_eq!(err, SwiftChainError::AlreadyInitialized.into()),
        _ => panic!(
            "Expected SwiftChainError::AlreadyInitialized, got {:?}",
            result
        ),
    }
}
...
#[test]
fn test_init_emits_protocol_initialized_event() {
    let (env, contract_id) = setup_env();
    let client = EscrowContractClient::new(&env, &contract_id);
    ...
    let topic_sym: Symbol = last_event.1.get(0).unwrap().into_val(&env);
    assert_eq!(topic_sym, Symbol::new(&env, "ProtocolInitialized"));

    let event_value: ProtocolInitialized = last_event.2.into_val(&env);
    assert_eq!(
        event_value,
        ProtocolInitialized {
            admin,
...
fn test_update_platform_fee_success() {
    ...
    init_contract(&env, &client, &admin, 0);
...
fn test_update_platform_fee_unauthorized() {
    ...
    let result = client.try_update_platform_fee(&malicious_user, &500);

    match result {
        Err(Ok(err)) => assert_eq!(err, SwiftChainError::Unauthorized.into()),
        _ => panic!("Expected SwiftChainError::Unauthorized, got {:?}", result),
    }
}
...
fn test_update_platform_fee_invalid_value() {
    ...
    match result {
        Err(Ok(err)) => assert_eq!(err, SwiftChainError::InvalidState.into()),
        _ => panic!("Expected SwiftChainError::InvalidState, got {:?}", result),
    }
}
...
#[test]
fn test_init_persists_protocol_config() {
    let (env, contract_id) = setup_env();
    let client = EscrowContractClient::new(&env, &contract_id);
    let admin = Address::generate(&env);

    let token = init_contract(&env, &client, &admin, 125);

    assert_eq!(client.get_admin(), admin);
    assert_eq!(client.get_token(), token);
    assert_eq!(client.get_platform_fee(), 125);
    assert_eq!(client.get_protocol_version(), 1);
}
...
fn test_happy_path_create_and_release() {
    ...
    client.init(&admin, &token_addr, &0);
...
fn test_release_escrow_insufficient_funds_rejected() {
    ...
    match result {
        Err(Ok(err)) => assert_eq!(err, SwiftChainError::InsufficientFunds.into()),
        _ => panic!("Expected SwiftChainError::InsufficientFunds, got {:?}", result),
    }
}
...
fn test_get_escrow_not_found() {
    ...
    match result {
        Err(Ok(err)) => assert_eq!(err, SwiftChainError::DeliveryNotFound.into()),
